...
public class Board extends GridPane {
	...
	public Board() {
		...
		for (int r = 0; r < 8; r++) {
			for (int c = 0; c < 8; c++) {
				if (squareArray[r][c].isFilled()) {
					//squareArray[r][c].getPiece().setOnMouseClicked(new PieceHandler()); JUST REMOVED TEMPORARILY
				}
			}
		}
	...
	/*
	private class SquareHandler implements EventHandler<MouseEvent> {

		@Override
		public void handle(MouseEvent event) {
			System.out.println("SQUARE HANDLER");
			if (event.getTarget().toString().equals("Square")) { //START OF SQUARE HANDLER
			Square currentSquare = (Square) event.getTarget();
			//System.out.println("This square is: " + currentSquare.isFilled());
			if (currentPiece == null) {
				//System.out.println("SELECT A PIECE FIRST");
				return;
			}
			...
			if (currentSquare.isFilled() && !currentSquare.getPiece().getColor().equals(currentPiece.getColor())) {
				if (defineLegalMove(squareArray, currentSquare)) {
					
					Square oldSquare = currentPiece.getPosition();
					oldSquare.removePiece();
					currentSquare.getPiece().setImage(null); //I AM JUST MAKING THIS INVISIBLE, it shouldnt be accessible but at this point you never know
					...
					takePiece(currentSquare.getPiece(), currentPiece, currentSquare, currentPiece.getPosition());
					unshowLegalMoves();
					updateBoardGUI(currentSquare);
				}
				return;
			}
			
			if (currentSquare.isFilled()) {
				if (currentPiece != null) {//make sure that one presses a square which isn't valid, deselect piece
			...
			if (currentSquare.getRankNum() > 7 || currentSquare.getFileNum() > 7 || currentSquare.getRankNum() < 0 || currentSquare.getFileNum() < 0) {
				System.out.println("SQUARE POSITION IS OUTSIDE BOARD");
				return;
			}
			
			//uses boolean to determine whether or not the selected square is a valid move.
			boolean temp = false;
			list = currentPiece.getLegalMoves(squareArray);
			for (int i = 0; i < list.size(); i++) {
				if (currentSquare.equals(list.get(i))){
					temp = true;
				}
			}
			...
			if (isInCheck(squareArray, currentPlayer)) {
			...
			if (game) {
				...
				//oldSquare.removePiece();
				/*
				currentPiece.getPosition().removePiece();
				currentSquare.setPiece(currentPiece);
				showLegalMoves();
				if (takePiece)
					updateBoardGUI(currentSquare);//important to remove piece when taken // actually not sure :(
				takePiece = false;
				//temp = false;
				...
				movePiece(currentPiece, currentSquare, currentPiece.getPosition());
				unshowLegalMoves();
				currentPiece.setEffect(null);
				currentPiece = null;
				currentSquare = null;
				list.clear();
				//I need to highlight the selected piece, but the issue is to either find a way to get the eventx and y and find the piece associated and
				//so that i can create a drag feature
				//otherwise just find a way to differentiate clicks from selecting the piece to moving, within the move method I should add the taking
				//feature so that it is all built in
			}
			}
		} 
		}
	...
	private class MouseHandler implements EventHandler<MouseEvent> {
		...
		public void handle(MouseEvent event) {
			...
			if (sideBar.getStartButtonState() == true) {
				for(int r = 0; r < squareArray.length; r++) {
					for (int c = 0; c < squareArray[0].length; c++) {
				...
				sideBar.setStartButtonState(false);
				
				squareArray = 
			} 
			
			if (sideBar.getPauseButtonState()) {
				game = false;
			} else if (!sideBar.getPauseButtonState()) {
				game = true;
			}
			if (game) {
...
*/
	...
	*/
			...
			for (int r = 0; r < squareArray.length; r++) {
				for (int c = 0; c < squareArray[0].length; c++) {
					if (squareArray[r][c].isFilled() && squareArray[r][c].getPiece().getColor().equals(currentPlayer))
						allMoves.addAll(validateLegalMoves(squareArray, currentPlayer, squareArray[r][c].getPiece()));
				}
			}
	...
	public LinkedList<Square> validateLegalMoves(Square[][] squareArr, Color player, Piece piece) {
		LinkedList<Square> potentialMoves = piece.getLegalMoves(squareArr, player);
	...
	public void showLegalMoves() {
		...
		//list = currentPiece.getLegalMoves(squareArray, currentPlayer);
			list = validateLegalMoves(squareArray, currentPlayer, currentPiece);
				for (int r = 0; r < 8; r++) {
					for (int c = 0; c < 8; c++) {
